...
A nostalgic, playable water ring-toss toy. Designed first for an iPhone held horizontally, with independent thumb pumps, 12 floating rings, timed challenges, optional fish, shared leaderboards, sound, and customizable artwork inside the tank.
...
- Finishing stops the timer and preserves the final tank scene without a win overlay. Enter a name above the toy and choose **SAVE**. **TIMES** shows the fastest ten names and completion times across devices, with separate boards for each of the four mode/fish combinations. Supabase stores shared scores; no player account is needed. Names and times are public. A finished round can be submitted once; retrying a save after a lost response reuses the same submission ID. An unavailable connection shows a retry message instead of claiming a score was saved. Keep the completed round open until saving succeeds. Previous device-only times stay on that device and are not automatically uploaded.
...
- Scene, photo, and sound preferences are saved locally where browser storage is available. Gameplay starts fresh on reload. Photos are resized and stored on the device; photos and device motion are never uploaded. Only a submitted name, time, challenge and random submission ID are sent to the shared leaderboard.
...
The game stays on GitHub Pages. `leaderboard.js` calls two Supabase RPCs without an external SDK. `leaderboard-config.js` contains the project URL and **publishable** key, which are intended for public browser use. Never use a secret/service-role key or database password in the repository.

To use another Supabase project:

1. Run `supabase/leaderboard.sql` in its SQL Editor. It creates the private score table and the read/submit functions; it is safe to rerun.
2. Set the URL and publishable key in `leaderboard-config.js`. Legacy public `anon` keys also work.
3. Increment the service worker cache version, publish, and verify all four boards.

The private table has RLS enabled and no direct anonymous table access. The API exposes only the top ten names/times for a challenge and validates submissions. Clients cannot edit or delete existing scores. UUIDs make retrying a submission idempotent; stable ordering resolves tied times. Offline play still works, but shared reads/saves require a connection. If both connection settings are empty, the game uses its original local-only boards with an explicit device-only label. An incomplete or invalid configuration shows an unavailable shared board.

This is a casual leaderboard: times come from the browser, so they are not cheat-proof. The public submission endpoint is not an abuse-proof service; use a server-verified run protocol and rate limiting before adding prizes or inviting heavy public traffic. The Supabase owner can moderate scores through the private table in the dashboard. There is no tracking or automatic upload of old local scores.
...
- `node tests/leaderboard.test.cjs`: shared API key handling, four-board reads, server-confirmed saves, retry identity, duplicate prevention, honest network failures, and stale-save isolation. These tests mock the network and never create production scores.
- `tests/leaderboard.sql`: PostgreSQL assertions for ranking, top ten limits, four-board isolation, invalid input, duplicate IDs and denied direct table reads/updates/deletes; run against a disposable database after the setup SQL. Its fixture transaction is rolled back.
